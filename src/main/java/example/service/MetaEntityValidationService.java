...
public class MetaEntityValidationService implements Validate<MetaEntity> {
    ...
    /**
     * Проверка соответствия атрибутов JPA метамодели
     * ПРАВИЛА:
     * - JPA атрибуты МОГУТ иметь MetaAttribute (не обязательно)
     * - MetaAttribute ДОЛЖНЫ иметь JPA аналоги (обязательно)
     */
    private List<String> validateJpaAttributes(EntityType<?> jpaEntity, MetaEntity metaEntity) {
        ...
        Map<String, Attribute<?, ?>> jpaAttributesMap = jpaEntity.getAttributes().stream()
        ...
        for (MetaAttribute metaAttribute : metaEntity.getAttributes()) {
            ...
            errors.addAll(validateAttributeTypeCompatibility(jpaAttribute, metaAttribute, metaEntity.getName()));
     ...
     */
    private List<String> validateAttributeTypeCompatibility(Attribute<?, ?> jpaAttribute,
     ...
     */
    private List<String> validateEnumCompatibility(Attribute<?, ?> jpaAttribute,
                                                   MetaAttribute metaAttribute,
                                                   String entityName) {
        List<String> errors = new ArrayList<>();
    ...
    private List<String> validateEntityLevelConstraints(MetaEntity metaEntity) {
        ...
        Set<String> attributeNames = metaEntity.getAttributes().stream()
            ...
            if (Boolean.TRUE.equals(attribute.getIsBidirectional()) &&
                ...
                if (attribute.getAttributeEntityType() != null &&
                    ...
                    errors.add(String.format(
                            ...
                            metaEntity.getName(), attribute.getName(),
                            related.getEntity().getName(), related.getName()
                    ));
                }
            }
        }
    ...
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(MetaEntityValidationService.class);
}