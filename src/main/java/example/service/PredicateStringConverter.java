...
public class PredicateStringConverter {
    ...
    private String convertAttributeReferenceToString(PredicateNode node) {
        ...
        return metaAttributeRepository.findById(node.getMetaAttribute().getId())
    ...
    private String convertPathExpressionToString(PredicateNode node) {
        ...
        if (pathExpression.getRootAttribute() != null) {
            String rootAttrName = metaAttributeRepository.findById(pathExpression.getRootAttribute().getId())
                    .map(MetaAttribute::getName)
        ...
        if (pathExpression.getPathAttributes() != null && !pathExpression.getPathAttributes().isEmpty()) {
            for (MetaAttribute attribute : pathExpression.getPathAttributes()) {
                String attrName = metaAttributeRepository.findById(attribute.getId())
                        .map(MetaAttribute::getName)
                        .orElse("MISSING_ATTR");
    ...
    public String convertToDebugString(PredicateDefinition predicateDefinition) {
        ...
        return String.format("Predicate[%s]: %s",
                predicateDefinition.getName(),
                convertToString(predicateDefinition)
        );
    }
}